...
namespace CommandIDs {
  export const copy = 'filemenu:copy';

  export const copyDownloadLink = 'filemenu:copy-download-link';

  // For main browser only.
  export const createLauncher = 'filemenu:create-main-launcher';

  export const cut = 'filemenu:cut';

  export const del = 'filemenu:delete';

  export const download = 'filemenu:download';

  export const duplicate = 'filemenu:duplicate';

  // For main browser only.
  export const hideBrowser = 'filemenu:hide-main';

  export const navigate = 'filemenu:navigate';

  export const open = 'filemenu:open';

  export const openBrowserTab = 'filemenu:open-browser-tab';

  export const paste = 'filemenu:paste';

  export const rename = 'filemenu:rename';

  // For main browser only.
  export const share = 'filemenu:share-main';

  // For main browser only.
  export const copyPath = 'filemenu:copy-path';

  export const showBrowser = 'filemenu:activate';

  export const shutdown = 'filemenu:shutdown';

  // For main browser only.
  export const toggleBrowser = 'filemenu:toggle-main';
}
...
const browser: JupyterLabPlugin<void> = {
  ...
  id: '@jupyterlab/filemenu-extension:browser',
  requires: [IFileMenuFactory, ILayoutRestorer],
  autoStart: true
};
 ...
 */
const factory: JupyterLabPlugin<IFileMenuFactory> = {
  activate: activateFactory,
  id: '@jupyterlab/filemenu-extension:factory',
  provides: IFileMenuFactory,
...
function activateFactory(
  ...
  const { commands } = app;
  const tracker = new InstanceTracker<FileMenu>({ namespace });
  const createFileMenu = (
    id: string,
    options: IFileMenuFactory.IOptions = {}
  ) => {
    const model = new FileMenuModel({
      manager: docManager,
      driveName: options.driveName || '',
      refreshInterval: options.refreshInterval,
      state: options.state === null ? null : options.state || state
    });
    const widget = new FileMenu({
      id,
      model,
  ...
  const defaultBrowser = createFileMenu('filemenu');

  return { createFileMenu, defaultBrowser, tracker };
}
...
function addCommands(
  app: JupyterLab,
  tracker: InstanceTracker<FileMenu>,
  browser: FileMenu
): void {
  const getBrowserForPath = (path: string): FileMenu => {
    ...
    if (driveName) {
      ...
      if (!browserForPath) {
        // warn that no filemenu could be found for this driveName
        console.warn(
          `${CommandIDs.navigate} failed to find filemenu for path: ${path}`
        );
        return;
      }
    ...
    // if driveName is empty, assume the main filemenu
    return browser;
  };
  const { commands } = app;
...
/**
 * Create a launcher for a given filemenu widget.
 */
